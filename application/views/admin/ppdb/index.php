...
function get_sort_url($column, $current_sort, $current_order) {
    ...
    return base_url('admin/ppdb?sort=' . $column . '&order=' . $new_order);
}

function get_sort_icon($column, $current_sort, $current_order) {
    if ($column != $current_sort) {
        return '<i class="fa-solid fa-sort text-[8px] opacity-20"></i>';
    }
    return $current_order == 'asc' 
        ? '<i class="fa-solid fa-sort-up text-[10px] text-brand-600"></i>' 
        : '<i class="fa-solid fa-sort-down text-[10px] text-brand-600"></i>';
}
?>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:bg-gray-100/50 transition-colors">
                            <a href="<?= get_sort_url('id', $sort, $order) ?>" class="flex items-center justify-between gap-2">
                                ID Pendaftar <?= get_sort_icon('id', $sort, $order) ?>
                            </a>
                        </th>
                        <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:bg-gray-100/50 transition-colors">
                            <a href="<?= get_sort_url('nama', $sort, $order) ?>" class="flex items-center justify-between gap-2">
                                Nama Lengkap <?= get_sort_icon('nama', $sort, $order) ?>
                            </a>
                        </th>
                        <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:bg-gray-100/50 transition-colors">
                            <a href="<?= get_sort_url('nisn', $sort, $order) ?>" class="flex items-center justify-between gap-2">
                                NISN <?= get_sort_icon('nisn', $sort, $order) ?>
                            </a>
                        </th>
                        <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:bg-gray-100/50 transition-colors">
                            <a href="<?= get_sort_url('tanggal', $sort, $order) ?>" class="flex items-center justify-between gap-2">
                                Tanggal Daftar <?= get_sort_icon('tanggal', $sort, $order) ?>
                            </a>
                        </th>
                        <th class="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:bg-gray-100/50 transition-colors">
                            <a href="<?= get_sort_url('status', $sort, $order) ?>" class="flex items-center justify-between gap-2">
                                Status <?= get_sort_icon('status', $sort, $order) ?>
                            </a>
                        </th>
        ...
        <div class="p-6 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between">
            <p class="text-xs text-gray-500 font-medium">
                Menampilkan <span class="font-bold text-gray-900"><?= $start ?></span> sampai <span class="font-bold text-gray-900"><?= $end ?></span> dari <span class="font-bold text-gray-900"><?= $total_rows ?></span> pendaftar
            </p>
            <?= $pagination ?>
        </div>
    </div>
